...
export class PaymentComponent implements OnInit {
  ...
  getCar() {
    this.carService.getCarDetail(this.rental.id).subscribe((response) => {
  ...
  totalPayment() {
    if (this.rental.returnDate != null) {
      ...
      this.amountOfPayment =
        numberOfDays * (this.car.dailyPrice + (this.car.dailyPrice * 8) / 100);

      if (this.amountOfPayment <= 0) {
        this.router.navigate(['/cars']);
        this.toastr.error(
          'You are being redirected to the car list',
          'Incorrect process'
        );
      }
    }
